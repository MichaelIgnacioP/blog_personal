...
class BlogListView(APIView):
    ...
    def get(self, request, format=None):
        if Post.postobjects.all().exists():
            ...
            return paginator.get_paginated_response({'results': {'posts': serializer.data}})
        else:
            return Response({'error': 'posts not exists'}, status=status.HTTP_404_NOT_FOUND)
...
class ListPostByCategoryView(APIView):
    ...
    def get(self, request, format=None):
        if Post.objects.all().exists():
            ...
            filtered_categories.extend(sub_categories)
